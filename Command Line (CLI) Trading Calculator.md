**Python** သုံးပြီး **Command Line (CLI) Trading Calculator** ကို တည်ဆောက်ပေးထားပါတယ်။

ဒီ Script မှာ -
- **Position Size** (Risk Management ကိုအခြေခံ)
- **Profit / Loss** (Pip, Lot နဲ့ တွက်)
- **Margin Required**
- **Risk/Reward Ratio**
- **Pip Value**
- **Margin Level & Free Margin** (အကောင့်အခြေအနေ)

စတဲ့ လုပ်ဆောင်ချက်အားလုံး ပါဝင်ပါတယ်။
...
### 1. Interactive Mode (Menu ပုံစံ)
Terminal ထဲမှာ `python trading_calc.py` (သို့မဟုတ် `python3 trading_calc.py`) ရိုက်ပါ။ ပြီးရင် ဂဏန်းတွက်ချက်လိုသည့် option ကို ရွေးပါ။

```
📊 Forex Trading Calculator (CLI)
========================================
1. Position Size (Risk based)
2. Profit / Loss
3. Required Margin
4. Risk / Reward Ratio
5. Pip Value
...
Risk per trade (%): 2
Stop Loss (pips): 20
Lot type (standard/mini/micro/nano) [micro]: micro

✅ You can trade: 0.1 micro lot(s)
   Risk amount: $20.00
```

### 2. Command Line Mode (Scripting အတွက်)

ဥပမာ - Position Size တွက်ရန်:
```bash
python trading_calc.py --mode pos --balance 1000 --risk 2 --sl 20 --lottype micro
```
အထွက်:
```
0.1
...
Profit/Loss တွက်ရန်:
```bash
python trading_calc.py --mode pl --pips 170 --lottype micro --lots 0.1
```
အထွက်:
```
1.70
...
Margin Level တွက်ရန်:
```bash
python trading_calc.py --mode marginlevel --balance 10000 --usedmargin 3000 --unrealized 1000
```
အထွက်:
```
Equity:11000.00,FreeMargin:8000.00,MarginLevel:366.67%
```
...
အထက်ပါ code ကို `trading_calc.py` လို့နာမည်ပေးပြီး သိမ်းပါ။ ပြီးရင် terminal မှာ အောက်ပါအတိုင်း run ပါ။

```bash
python3 trading_calc.py
```

ဒီ script မှာ **argparse** နဲ့ **interactive menu** နှစ်မျိုးလုံးပါတာမို့ သင့်စိတ်ကြိုက် သုံးလို့ရပါပြီ။

နောက်ထပ် လိုအပ်တဲ့ အကြောင်းအရာရှိရင် ပြောပါ။
